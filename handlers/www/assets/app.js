    ...
    app.controller('PlayController', ['$scope', '$rootScope', '$log', '$http', '$location', '$timeout', '$mdDialog', '$window', 'TerminalService', 'KeyboardShortcutService', 'InstanceService', 'SessionService', 'Upload', function ($scope, $rootScope, $log, $http, $location, $timeout, $mdDialog, $window, TerminalService, KeyboardShortcutService, InstanceService, SessionService, Upload) {
        ...
        $scope.uploadFiles = function (files, invalidFiles) {
            ...
            let uploadFile = function () {
                ...
                let upload = Upload.upload({
                    url: '/sessions/' + $scope.sessionId + '/instances/' + $rootScope.selectedInstance.name + '/uploads',
                    data: {file: file},
                    method: 'POST'
                })
                    .then(function () {
        ...
        $scope.setSessionState = function (state) {
            ...
            if (!state) {
                $mdDialog.show({
                    onComplete: function () {
                        SessionBuilderModalController($mdDialog, $scope)
                    },
        ...
        $scope.getSession = function (sessionId) {
            $http({
                ...
                if (window.location.port) {
                    ...
                    // console.warn(`[web_socket] For base "${base}" also used port "${window.location.port}". Now - this ignored.`)
            ...
            term.attachCustomKeyEventHandler(function (e) {
                ...
                var presets = selectedKeyboardShortcuts.presets
                    .filter(function (preset) {
                        return preset.keyCode == e.keyCode
                    })
                    .filter(function (preset) {
                        return (preset.metaKey == undefined && !e.metaKey) || preset.metaKey == e.metaKey
                    })
                    .filter(function (preset) {
                        return (preset.ctrlKey == undefined && !e.ctrlKey) || preset.ctrlKey == e.ctrlKey
                    })
                    .filter(function (preset) {
                        return (preset.altKey == undefined && !e.altKey) || preset.altKey == e.altKey
                    })
                    .forEach(function (preset) {
                        preset.action({terminal: term, e})
                    });
            });
